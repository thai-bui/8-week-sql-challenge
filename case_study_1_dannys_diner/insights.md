...
## 🎯 Project Goal
To analyze Danny’s Diner’s early customer, sales, and menu data in order to uncover meaningful patterns about customer visits, spending behavior, and menu preferences. The goal is to transform the raw datasets into actionable insights that can help Danny strengthen customer relationships and improve overall restaurant performance. These insights will guide decisions about enhancing the loyalty program and optimizing day-to-day operations. SQL (PostgreSQL) is used to build datasets, answer business questions, and support data-driven decision-making.
...
| **data/** | Placeholder folder for raw CSV or data files (not strictly required for this small dataset). |
